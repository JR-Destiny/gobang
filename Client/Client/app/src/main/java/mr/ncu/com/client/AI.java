...
public class AI implements Runnable {
    ...
    //无棋
    private int NO_CHESS=0;
    //白棋
    private int WHITE_CHESS=1;
    //黑棋
    /*
     * 评分表（落子优先级评分）
     * FIVE 至少能五子相连
    ...
    public void run() {
        ...
        pos=new Point(max.getX(),max.getY());
    ...
    private int getHorizontalPriority(int x, int y, int chess) {
        ...
        if (y == 0) {
            ...
            for (int i = y - 1; i >= 0; i--) {
                ...
                if (chessArray[x][i] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isStartStem = chessArray[x][i] != NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == 0) {
        ...
        if (y == panelLength - 1) {
            ...
            for (int i = y + 1; i < panelLength; i++) {
                ...
                if (chessArray[x][i] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isEndStem = chessArray[x][i] != NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == panelLength - 1) {
    ...
    private int getVerticalPriority(int x, int y, int chess) {
        ...
        if (x == 0) {
            ...
            for (int i = x - 1; i >= 0; i--) {
                ...
                if (chessArray[i][y] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isStartStem = chessArray[i][y] != NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == 0) {
        ...
        if (x == panelLength - 1) {
            ...
            for (int i = x + 1; i < panelLength; i++) {
                ...
                if (chessArray[i][y] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isEndStem = chessArray[i][y] != NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == panelLength - 1) {
    ...
    private int getLeftSlashPriority(int x, int y, int chess) {
        ...
        if (x == 0 || y == 0) {
            ...
            for (int i = x - 1, j = y - 1; i >= 0 && j >= 0; i--, j--) {
                ...
                if (chessArray[i][j] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isStartStem = chessArray[i][j] != NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == 0 || j == 0) {
        ...
        if (x == panelLength - 1 || y == panelLength - 1) {
            ...
            for (int i = x + 1, j = y + 1; i < panelLength && j < panelLength; i++, j++) {
                ...
                if (chessArray[i][j] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isEndStem = chessArray[i][j] != NO_CHESS;
                    break;
                } else {
                    connectCount++;
